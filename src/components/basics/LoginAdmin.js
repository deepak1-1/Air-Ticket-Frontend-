...
const LoginAdmin = (props)=> {
    ...
    function handleLogin(e){
        ...
        if((userData.email.trim().length !== 0) && (userData.password.trim().length !== 0)){
            ...
            axios.post( basicInfo_.url + 'login-admin', userData ) //to check wether user exists or not
            .then(res => {
    ...
    function handleGetCode(e){
        ...
        if(e.target.innerText === 'Get Code'){
            if(check_regex_match(email_exp, modalData.email)){
                ...
                axios.post( 'http://localhost:4000/send-code',{email: modalData.email, data: "admin"})
                    .then(res=>{
            ...
            if(check_regex_match(password_exp, modalData.password)){
                ...
                if(modalData.password === modalData.confirm_password){
                    ...
                    axios.post('http://localhost:4000/update-password-admin',
                                {email: modalData.email, password: modalData.password})
                        .then(result => {
                ...
                <div className="col-lg-6 offset-3 text-left border border-secondary p-sm-2 p-lg-5 rounded">
                    ...
                    <form >
                        ...
                        <div className="col-8 offset-2">
                            <button type="button" className="col-6 offset-3 btn btn-outline-primary" onClick={handleLogin}><b>Admin login</b> <i className="fas fa-arrow-right"></i></button>
                        </div>
                        ...
                        <div className="container signin text-center mt-2">
                            <p className="mb-0 pb-0">New User? <a href="/">register</a> or login as <a href='/login'>User</a>.</p>
